...
public class EntitySearchScroller implements QueryPaginator
{   
	...
	EntitySearchScroller(EntityIndex index, IndexSearcher searcher, String userQuery, Query decoratedQuery) throws Exception
	{
		m_index = index;
		m_searcher = searcher;
	...
	public IndexedScrollerPage readPage(int pageNumber) throws Exception
	{
		...
		for (int n=start; n<=end&&n<rowCount;  n++) {
			...
			hit.setSummary(getSummary(handler, entity));
			...
			if ("PublicCommunity".equals(typeName) || "ProtectedCommunity".equals(typeName)) {
				...
				hit.setIsMember(isMember(com, getCurrentUser()));
				hit.setIsMembershipRequested(isMembershipRequested(com, getCurrentUser()));
			} else {
				if ("BlogEntry".equals(typeName)) {
					...
					user = ft.getAuthor();
				} else if ("ForumResponse".equals(typeName)){
					ForumResponse fr = (ForumResponse)entity;
					com = fr.getForum().getCommunity();
					user = fr.getAuthor();
					...
					if (getCurrentUser() != null && getCurrentUser().getId() > 0) {
						if (getCurrentUser().getId() == user.getId()) {
							hit.setConnectionInfo("");
						} else {
							Contact contact = null;
							try {
								contact = user.getContact(user.getId(), getCurrentUser().getId());
							} catch (ElementNotFoundException e) {
								String returnString = "<a onClick='addConnection("+user.getId()+", \""+user.getFullName()+"\");' href='javascript:void(0);'" +
								"title='Add "+user.getFullName()+" to my connections' class='search_btn right' style='font-size:12px;'>Add to my connections</a>";
								hit.setConnectionInfo(returnString);
							}
							if (contact != null && contact.getId() > 0) {
								hit.setContactId(contact.getId());
								hit.setContactStatus(contact.getStatus());
								hit.setConnectionInfo(getConnectionInfo(user, getCurrentUser(), contact));
							}
						}
					} else {
						hit.setConnectionInfo("");

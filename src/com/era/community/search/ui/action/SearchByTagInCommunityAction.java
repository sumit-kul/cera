...
/**
 * @spring.bean name="/cid/[cec]/search/searchByTagInCommunity.do"
 * @spring.bean name="/cid/[cec]/search/searchByTagInCommunity.ajx"
 */
public class SearchByTagInCommunityAction extends AbstractCommandAction
{
	...
	protected ModelAndView handle(Object data) throws Exception
	{
		...
		if (comm != null) {
			cmd.setCummunityId(Integer.toString(comm.getId()));
		}
		...
		if (context.getRequest().getParameter("rmFilterTag") != null 
			...
			if (!cmd.getFilterTagList().contains(",")) {
				return new ModelAndView("redirect:/cid/"+cmd.getCummunityId()+"/home.do");
			}
		...
		scroller = tagFinder.getAllItemsByTagInCommunity(comm.getId(), cmd.getFilterTagList(), cmd.getSubmitType());
		if (pageNum != null) {
	...
	public class RowBean 
	{
		...
		public String getTaggedKeywords(int pageNumber, String filterTag, String sortOption, String toggleList){
			...
			try {
				List tags = tagFinder.getTagsForParentTypeByPopularity(this.getItemId(), this.getCommunityid(), 20, this.getItemType());
				...
				for (Iterator iterator = tags.iterator(); iterator.hasNext();) {
					...
					taggedKeywords += "<a href='cid/"+this.getCommunityid()+"/search/searchByTagInCommunity.do?filterTag="+tag+filterTagString+"&toggleList="+toggleList+" ' class='euInfoSelect' style='display: inline;' onmouseover='tip(this,&quot;Click to filter by tag &#39;"+tag+"&#39;&quot;)'>"+tag+"</a>";
		...
		public String getDisplayItemType() throws Exception
		{
			if (this.getItemType().contains("Topic"))
				return "Forum";
			else if (this.getItemType().contains("Response"))
				return "Forum";
			else if (this.getItemType().contains("Wiki"))
				return "Wiki";
			else if (this.getItemType().contains("Blog"))
				return "Blog";
			else if (this.getItemType().contains("Member") | this.getItemType().contains("Community Admin") )
				return "Member";
			else
		...
		public String getItemUrl() throws Exception
		{
			...
			if (this.getItemType().endsWith("File")) {
	...
	public static class Command extends IndexCommandBeanImpl
	{
		...
		public String getDisplayedFilterTag() {
			...
			if (getFilterTagList() != null && !"".equals(filterList)) {
				...
				while (st.hasMoreTokens()) {
					String tag = st.nextToken().trim().toLowerCase();
					returnHtmlTags += "<a href='cid/"+getCummunityId()+"/search/searchByTagInCommunity.ajx?fTagList="+filterList+"&rmFilterTag="+tag+"' class='euInfoRmTag' style='display: inline;' title='Remove "+tag+" filter'>"+tag+" <span style='color: rgb(137, 143, 156); font-size: 12px;' title='Remove "+tag+" filter' >X</span></a>";
				}
			}
		...
		public String getCummunityId() {
			return cummunityId;
		}

		public void setCummunityId(String cummunityId) {
			this.cummunityId = cummunityId;
		}

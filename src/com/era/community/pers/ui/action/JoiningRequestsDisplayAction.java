...
/**
 *  @spring.bean name="/pers/joiningRequests.do" 
 *  @spring.bean name="/pers/joiningRequests.ajx"
 */
public class JoiningRequestsDisplayAction extends AbstractCommandAction
{
	...
	protected ModelAndView handle(Object data) throws Exception
	{
		...
		/* Get list of all joining requests for communities current user has an admin role for*/
		QueryScroller scroller = communityJoiningRequestFinder.listJoiningRequestForAdmin(currUser, cmd.getSortByOption());
		//scroller = communityJoiningRequestFinder.listMySentJoiningRequest(currUser, cmd.getSortByOption());
		...
		if (pageNum != null) {
			...
			return new ModelAndView("/pers/showJoiningRequests");
		}
	}
	...
	public class Command extends IndexCommandBeanImpl
	{
		...
		public List getSortByOptionOptions() throws Exception
		{
			...
			sortByOptionList.add("Name Of Requestor");
	...
	public class RowBean extends CommunityJoiningRequestDto implements EntityWrapper
	{
		...
		public String getRequestInfo() throws Exception
		{
			...
			try {
				...
				if (jr.getStatus() == 0) {
					returnString = "<a onClick='handleMemberRequest("+this.getRequestId()+ ", "+2+")' href='javascript:void(0);' "+
					"title='Reject membership request from "+this.getRequester()+"' class='btnmain normalTip' >Reject</a>"+
					"<a onClick='handleMemberRequest("+this.getRequestId()+ ", "+1+")' href='javascript:void(0);' " +
					"title='Accept membership request from "+this.getRequester()+"' class='btnmain normalTip' >Accept</a>";
				}
